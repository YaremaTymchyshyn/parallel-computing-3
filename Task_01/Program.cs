...
namespace Task_01
{
    class Program
    {
        static void Main()
        {
            const int height = 331;
            const int width = 3545;
            ...
            Console.WriteLine($"Time spent on sequential addition: {finish - start}\n");
        ...
        public static int[,] AddMatricesAdaptive(int[,] matrix1, int[,] matrix2, int threadNum, int height, int width)
        {
            ...
            if (width > height)
            {
                ...
                for (int i = 0; i < threadNum; i++)
                {
                    ...
                    Thread thread = new Thread(() => AddMatricesInArea(matrix1, matrix2, result, 0, height, from, to));
                    threads.Add(thread);
                }
            }
            else
            {
                ...
                for (int i = 0; i < threadNum; i++)
                {
                    ...
                    Thread thread = new Thread(() => AddMatricesInArea(matrix1, matrix2, result, from, to, 0, width));
                    threads.Add(thread);
                }
            }
            ...
            var finish = DateTime.Now;
            Console.WriteLine($"Time spent on parallel addition using {threadNum} threads: {finish - start}");
            return result;
        }
